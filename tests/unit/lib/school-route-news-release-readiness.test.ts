...
describe("school route news release readiness regressions", () => {
  it("keeps the newest editorial items at the top of the school-route-news feed", () => {
    ...
    expect(allNewsItems[1]?.slug).toBe("hiroshima-city-photographing-and-approach-20260915")
    expect(allNewsItems[2]?.slug).toBe("saitama-city-child-approach-cluster-20260915")
    expect(allNewsItems[3]?.slug).toBe("national-student-bicycle-commute-accidents-20260912")
    expect(allNewsItems[4]?.slug).toBe("kumamoto-statewide-child-approach-cluster-20260910")
