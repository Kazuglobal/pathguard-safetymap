...
vi.mock("@/lib/danger-report-moderation-ai", () => ({
  DANGER_MODERATION_PROMPT_VERSION: "v2",
...
describe("moderateDangerReportRecord", () => {
  ...
  it("uses status and moderation guards for a live approval", async () => {
    ...
    expect(admin.update.calls).toContainEqual(["eq", "title", report.title])
    ...
    const selectCall = admin.update.calls.find(([name]) => name === "select")
    expect(selectCall?.[1]).not.toBe("*")
    expect(String(selectCall?.[1])).not.toContain("ai_moderation_reason")
    expect(String(selectCall?.[1])).not.toContain("ai_moderation_score")
  })

  it("uses IS NULL when guarding a nullable description", async () => {
    const admin = adminFor("live")

    await moderateDangerReportRecord({
      supabaseAdmin: admin.client,
    ...
    expect(admin.update.calls).toContainEqual(["is", "description", null])
  })

  it("keeps a live fallback pending so the sweeper can retry it", async () => {
    const admin = adminFor("live")
    vi.mocked(moderateDangerReportWithAi).mockResolvedValueOnce({
      status: "needs_review",
      reason: "AI審査に失敗しました",
      score: 0.5,
      aiExecuted: false,
    ...
    expect(admin.from).toHaveBeenCalledTimes(4)
    expect(admin.update.calls).toEqual([])
  })
...
describe("getDangerModerationFallbackCount", () => {
  it("counts only live fallbacks from the current prompt version", async () => {
    const query = builder({ count: 2, error: null })
    const supabaseAdmin = {
      from: vi.fn(() => query),
    }

    const count = await getDangerModerationFallbackCount(
      supabaseAdmin as any,
      "report-1",
    )

    expect(count).toBe(2)
    expect(query.calls).toContainEqual(["eq", "mode", "live"])
    expect(query.calls).toContainEqual(["eq", "prompt_version", "v2"])
  })
})

describe("markDangerReportModerationFailed", () => {
  it("returns the transitioned report and notifies its submitter", async () => {
    const failedReport = {
      ...report,
      ai_moderation_status: "needs_review",
    }
    const query = builder({ data: failedReport, error: null })
    const supabaseAdmin = {
      from: vi.fn(() => query),
    }

    const result = await markDangerReportModerationFailed(
      supabaseAdmin as any,
      "report-1",
      new Date("2026-07-18T00:00:00.000Z"),
    )

    expect(result).toEqual(failedReport)
  ...
  it("returns null when the conditional needs-review transition loses a race", async () => {
    ...
    expect(query.calls).toContainEqual(["select", expect.any(String)])
    expect(query.calls).toContainEqual(["maybeSingle"])
  })
})
