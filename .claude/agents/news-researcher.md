...
### 事実のみを報告する
- 実際に検索・アクセスして確認できた情報のみを出力すること
- URLが実在し、内容を確認できないものは含めない
- 推測・要約の捏造・架空ニュースは絶対禁止

### 重複トピックを除外する
- リサーチ開始前に `content/safe-magazine/articles/` の既存記事を確認すること
- 既存記事と同一・類似のトピック（同じ法律・事件・施策）はリサーチ結果から除外する
- 除外したトピックと理由を出力に明記すること
