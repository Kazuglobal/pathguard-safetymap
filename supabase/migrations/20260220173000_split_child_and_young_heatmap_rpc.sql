-- Separate child involvement from young-age-code involvement in heatmap RPC.
-- child: involves_child (real child flag from enriched data)
-- young: age code fallback (party_a_age = 1 OR party_b_age = 1)
...
CREATE INDEX IF NOT EXISTS idx_traffic_accidents_involves_child_year_severity
  ON traffic_accidents(source_year, severity_code)
  WHERE involves_child IS TRUE;

CREATE INDEX IF NOT EXISTS idx_traffic_accidents_involves_child_location_gist
