...
> {
  ...
  try {
    // redirect: "manual" — リダイレクト先URLは validateImageUrl の検証
    // (https強制・プライベートIP拒否・allowlist)を経ないため、追従を許すと
    // 攻撃者サーバの 302 で内部エンドポイントへ誘導される SSRF 経路になる
    imageResponse = await fetch(imageUrl, { method: "GET", signal, redirect: "manual" })
  } catch (error) {
  ...
  if (
    imageResponse.type === "opaqueredirect" ||
    (imageResponse.status >= 300 && imageResponse.status < 400)
  ) {
    return {
      ok: false,
      statusCode: 400,
      error: "Redirected image URLs are not supported",
    }
  }
