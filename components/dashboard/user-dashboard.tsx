...
export default function UserDashboard() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (active) {
          setName(profile?.display_name || user.email?.split("@")[0] || "ユーザー")
  ...
  const waitingCount = reports.filter((report) => ["pending", "reviewing"].includes(report.status)).length
      ...
      <div className="mx-auto max-w-5xl space-y-5">
        ...
        <section className="grid gap-4 sm:grid-cols-3" aria-label="自分の状況">
          ...
          <PaperPanel tone="green" className="p-5">
            ...
            <p className="text-3xl font-black">{loading ? "—" : `${reports.length}件`}</p>
          ...
          <PaperPanel tone="accent" className="p-5">
            ...
            <p className="text-3xl font-black">{loading ? "—" : `${waitingCount}件`}</p>
