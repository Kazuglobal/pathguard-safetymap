...
## 第3ラウンド: 修正そのものへの敵対的レビュー(2026-07-30)

対象を「まだレビューを通っていない直近2コミット」(41309bea6=第2ラウンドの修正 / 9b5b47a9d=アイコン)に
絞って実行。**修正が新たな欠陥を生んでいないか**を重点にした結果、第2ラウンドの修正のうち
2件が不完全、1件が完全な no-op だったことが判明した。

### CONFIRMED(3件。いずれも自分で再現を取ってから修正)
- [修正] **IPv6 の SSRF 判定が死んでいた**。`[::ffff:127.0.0.1]` は WHATWG URL が
  **16進ピースへ正規化**して `[::ffff:7f00:1]` を返すため、ドット付き十進を期待した
  自分の正規表現は一度も一致しない死に分岐だった(= ループバック・プライベート宛先が素通り)。
  → 判定を `supabase/functions/analyze-hazard/url-guard.ts`(Deno非依存の純関数)へ切り出し、
  16進ピースから IPv4 を復元して判定。**vitest で実挙動テスト28件**を新設
  (model-config.ts と同じ既存パターンに倣った)
- [修正] **画像生成の匿名化ガードが prompt 空のとき外れる**。`imageBase64 && prompt?.trim()` を
  条件にしていたが、`lib/openai-image.ts` は prompt 空なら既定文へフォールバックして
  画像編集を実行するため、ガードだけが外れる。→ 条件を**元写真の有無だけ**に変更。
  standard 経路(applyGuard)にも同じ欠陥があったので合わせて修正
- [修正] **クラスタの当たり判定限定が完全な no-op だった**。`.danger-cluster-pin` は
  `inset:0` で箱全体を覆うため、SVGルートに `auto` を戻した時点で全面が復活していた

### 実ブラウザで確定させた事実(今回の肝)
**SVGルート要素は透明部分も含めて箱全体で当たり判定を取る**(Chrome実測)。
`pointer-events: visiblePainted` を指定しても変わらない。正しいのは
**「ルート=none / 内側の図形=auto」**。この方式なら透明部は下へ抜け、ピンの形そのものが
当たり判定になる(個別ピンも従来の48×56矩形からティアドロップ形状へ改善)。
検証は `scripts/verify-marker-hit-area.mjs` としてリポジトリに残した。
ミューテーション(`.danger-cluster-pin` を auto に戻す)で失敗することも確認済み。

### PLAUSIBLE への対応
- [修正] hazard-game/analyze の 429 を呼び出し側が `console.warn` するだけで、
  ハザードゼロのまま画像生成(最大5回の従量課金)まで進んでいた
  → 429 はユーザーに表示して中断。回帰テスト追加
- [修正] generate-pwa-icons.mjs の三重ネスト三項を `resolveSource()` に集約
  (再生成して出力がバイト同一=挙動不変を確認)
- [見送り] icon-source-maskable.svg がアートを複製している件: SVGは静的アセットで、
  変更時は両方を再生成する運用。ビルド時合成を入れるほどの頻度ではない

### 学び(再発防止)
- **テキスト照合だけのテストを「検証」と呼ばない**。第2ラウンドで書いた
  「ソースに識別子が含まれるか」を見るテストは、判定が死んでいても緑だった。
  Deno等でimportできない場合は**純関数を別モジュールへ切り出して実挙動をテストする**
  (既に model-config.ts で確立していたパターンを自分が使わなかった)
- **ブラウザ挙動の思い込みを実測で潰す**。pointer-events の効き方を確かめずに
  修正した結果、片方は no-op、もう片方も不完全だった
- レビュー対象を「未レビューのコミット範囲」に明示的に絞ると、**修正そのものの欠陥**が出る

## アイコン正式アートの作成(2026-07-29)

A2 の残作業を完了。**Codex CLI(ChatGPT サブスクリプション)で作成し、画像生成APIは不使用**。

- 拡散モデルの描画ではなく **SVG ベクターアート**として作らせた。アプリアイコンは 1024px〜96px まで
  劣化なしで縮む必要があり、maskable 版を別描画できるためパディング由来の継ぎ目も出ない
- 実行は `codex exec --cd <scratchpad> --sandbox workspace-write` でリポジトリ外に隔離し、
  プロンプトに「作業ディレクトリ外を読まない・変更しない・削除しない / npm install しない /
  ネットワーク取得しない」を明記(サブエージェント破壊事故の規律をCodexにも適用)
- **3回反復した**。Codex の自己申告を信用せず、毎回自分でラスタライズして目視確認:
  ①ピンと緑円がシールドの輪郭をはみ出していた → 修正 ②点線が接触して「毛虫」状になり、
  終点がピンから垂れ下がった → 修正指示 ③Codex が「ドット径48pxで6個は幾何学的に入らない」と
  正しく指摘 → 96pxで消える縮小案を却下し、**ドット数を4個に減らす**判断を返して決着
- `scripts/generate-pwa-icons.mjs` を SVG 入力対応に拡張(優先順: icon-source.svg →
  icon-source.png → 暫定SVG)。maskable 専用SVGがあれば直接描画する
- 生成物を実測確認: 192/512/180/96 の各サイズ、maskable はセーフゾーン80%、
  apple-touch-icon は不透明(alpha=false)
