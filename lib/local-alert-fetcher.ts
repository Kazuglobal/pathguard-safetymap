...
[
  {
    ...
    "occurred_at": "ISO8601形式の発生日時（不明な場合は本日のJST 12:00）"
  }
]
...
export async function fetchLocalAlertsFromGemini(): Promise<LocalAlertInput[]> {
  let apiKey: string
  try {
  ...
  return parseGeminiAlertResponse(text)
}
