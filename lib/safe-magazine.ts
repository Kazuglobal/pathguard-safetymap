...
export const ARTICLES: SafeMagazineArticle[] = [
  {
    ...
    excerpt: "9月21日から30日まで実施される「令和8年秋の全国交通安全運動」の全国重点3番目は、自転車・特定小型原動機付自転車の交通ルール徹底です。推進要綱は特定小型原付の対歩行者事故の割合が自転車の約3倍と指摘しています。自転車ヘルメットの全国着用率21.2%という現実をふまえ、今週末15分でできる3つの見直しをまとめました。",
    ...
    keyPoints: [
      ...
      "特定小型原動機付自転車は免許不要だが16歳未満の運転は禁止で、16歳未満に貸す・買い与えることも禁止（罰則: 6月以下の拘禁刑または10万円以下の罰金）。保護者側が禁止行為に当たる点に注意",
      "今週末5分、ヘルメットの「サイズ・あごひも・傷/使用年数」を手に取って確認する。東京都内では非着用時の致死率が着用時の約2.3倍（警視庁・令和3〜7年）"
    ],
    sources: [
      "内閣府『令和8年秋の全国交通安全運動推進要綱』（令和8年7月1日 中央交通安全対策会議 交通対策本部決定）[公式発表]（実施期間: 2026年9月21日〜9月30日）",
