...
type FetchFromXRoadOptions = { proxyOrigin?: string }
 ...
 * @returns APIレスポンス
 */
function resolveProxyOrigin(proxyOrigin?: string): string {
  if (proxyOrigin) return proxyOrigin;
  if (typeof window !== 'undefined') return window.location.origin;
  throw new Error('proxyOrigin is required when calling xROAD from server runtime');
}

export async function fetchFromXRoad(url: string, options: FetchFromXRoadOptions = {}) {
  ...
  const proxyUrl = new URL(PROXY_API_BASE_URL, resolveProxyOrigin(options.proxyOrigin)); // 第2引数で絶対URLを生成
...
export async function getRoadData(
  ...
  try {
    ...
    // --- デバッグログ追加（緯度経度は丸めてPIIを避ける） ---
    const roundCoord = (n: number) => Math.round(n * 100) / 100;
    console.log("[getRoadData] Input params:", {
    ...
    console.log("[getRoadData] Calculated BBOX:", {
    ...
    console.log(finalUrl.origin + finalUrl.pathname + " (cql_filter に緯度経度が含まれるためログには出力しない)");

    const data = await fetchFromXRoad(finalUrl.toString(), options);
    return data;
...
export async function getTrafficData(
  ...
  try {
    ...
    // APIリクエストを実行
    const data = await fetchFromXRoad(url.toString(), options);
    return data;
  } catch (error) {
