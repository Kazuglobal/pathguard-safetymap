...
- suspicious も共通パイプラインへ移行し、自動承認にはAI呼び出しの成功を必須とした。`off` / `shadow` 中はDBを公開状態へ更新せず、投稿者画面にも「確認中」と表示する。liveでAPIキー未設定、タイムアウト、壊れた応答などが起きた場合は最大3回再試行後に `needs_review` へ送る。
...
2. ローカルSupabaseで `supabase test db` を実行し、一般ユーザーのAI審査列UPDATEが42501、通常本文編集とadmin更新が成功することを確認する。
3. `CRON_SECRET` とGemini APIキーを設定する。
4. `DANGER_REPORT_AI_MODERATION_MODE=off` のままデプロイし、cron認証と管理画面を確認する。
5. `pnpm run moderation:backtest -- --limit 100` を並列度1で実行する。危険側誤り0件かつapprove再現率30%以上の場合だけ次へ進む。
6. `shadow` で2週間または100件以上を収集し、危険側誤り率1%未満を確認する。
7. 条件を満たした場合だけ `live` へ切り替える。精度劣化時はまず `shadow` へ戻す。

環境変数の既定値は `off`。値が不正な場合も `off` として扱う。
shadowのスイーパーは同じレポートを同じ `prompt_version` で1回だけ評価する。プロンプト改訂後に再収集する場合は `DANGER_MODERATION_PROMPT_VERSION` を更新する。
