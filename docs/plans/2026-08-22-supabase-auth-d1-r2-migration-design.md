...
| 項目 | 決定 |
|---|---|
| 認証 | **Supabase Auth を継続**(Google OAuth / LINE カスタム / パスワード / PKCE コールバック / `@supabase/ssr` Cookie セッションはそのまま) |
...
| `.storage.from()` 呼び出し | 19 箇所(うち **クライアント直アップロード 2**、クライアント直削除 3) |
...
- `components/admin/ProcessImageDialog.tsx:77` は `processed_images`(アンダースコア)を指しており、定義上のバケット `processed-images` と不一致(現状バグの疑い)
...
ビュー 4 本(`danger_reports_public_preview`, `report_stats`, `public_reports_with_stats`, `danger_category_stats`)は SQLite の VIEW で再定義可能だが、`danger_reports_public_preview` は「RLS を迂回する owner ビュー」なので、D1 では**リポジトリ関数 `listPublicPreview()`(座標 0.01° 丸め)として実装**する。
...
破棄前に **pg_dump を R2 の `backups/` に保存**(§10.1)。`danger_reports` の FK(`address_municipalities`, `address_prefectures`)は D1 では FK を張らず文字列列として残す。

### 2.3 Supabase に残すもの

- Auth(users, identities, sessions, OAuth 設定, JWT 署名鍵)
- `supabase/functions/analyze-hazard` は **廃止して Workers の Route Handler に統合**(既に `app/api/hazard-game/analyze` 等の同等ルートがあり、Deno 版は D1 に届かない)
...
Browser ──(Cookie: sb-*)──▶ Next.js on Cloudflare Workers (OpenNext)
                                │  ├─ middleware: Supabase JWT をローカル検証(JWKS)
...
| P0-4 | `web-push` / `jose` / `openai` / `@anthropic-ai/sdk` が `nodejs_compat` で動く | 不可なら fetch ベース実装へ |
...
| トリガ `updated_at` | Drizzle のリポジトリ層で `updatedAt: now()` を必ずセット(SQLite トリガも可だが二重管理を避ける) | |
...
- **ブラウザ直アクセスの置換**: `hooks/use-*.ts` 46 箇所 + `components/*` 11 箇所は、`app/api/**` の Route Handler(または Server Action)を叩く fetch に置き換える。`hooks/` の公開インターフェース(戻り値・state 名)は維持し、characterization テスト(`tests/unit/hooks/*`)で固定してから差し替える
- 14 RPC の対応:
...
| クエリ 30 秒 / 単一スレッド | `get_accidents_in_bbox` の 10,000 行 | インデックス + `LIMIT`、上限を 5,000 に下げる。JSON 組み立ては JS 側 |
| Time Travel 30 日 | バックアップ | 加えて日次 `wrangler d1 export` を R2 `backups/` へ(Cron Trigger) |

---

## 6. 空間クエリの再実装(PostGIS 代替)

原則: **D1 で bbox 粗選別 → JS(`@turf/*`、既に `types/turf.ts` があり依存追加可)で厳密判定**。
...
- `Actor` の生成は `lib/auth/actor.ts` 一箇所: Cookie のアクセストークンを **JWKS(`https://<project>.supabase.co/auth/v1/.well-known/jwks.json`)でローカル検証**(`jose`、10 分キャッシュ)→ `{ id: sub, email, isAdmin: isAdminEmail(email) || app_metadata.role==='admin' }`。有効期限切れ時のみ `@supabase/ssr` の `getUser()` でリフレッシュ(ミドルウェアは現行のまま)
  - 前提作業: Supabase ダッシュボードで JWT 署名鍵を **ES256 へ移行**(レガシー HS256 は段階的に失効)。Cookie 形式・`@supabase/ssr` は変更不要
- `profiles.role = 'admin'` は RLS からしか使われておらず、アプリ層は `ADMIN_EMAILS`(`lib/admin.ts`)で判定済み。D1 では **`ADMIN_EMAILS` を唯一の真実**とし、`profiles.role` は表示用に残すだけ(権限昇格経路を増やさない)

### 7.2 ポリシー → ルール表(抜粋。全テーブル分を `lib/db/authz/matrix.ts` に定数化)

| テーブル | select | insert | update | delete |
|---|---|---|---|---|
| `danger_reports` | anon: `listPublicPreview` のみ(丸め座標)。user: `status ∈ PUBLIC` or 自分。admin/service: 全件 | user: `user_id = self` かつ `status='pending'` 固定(`20260704090000`) | user: 自分の行、moderation/status 列は不可。admin: status 可。service: 全列 | user: 自分。admin: 全件 |
...
| `user_routes` / `route_learning_sessions` / `push_subscriptions` / `user_points` / `user_badges` / `user_mission_progress` / `safety_quest_attempts` | 自分 | 自分 | 自分 | 自分 |
...
| `pg-media-private` | `danger-reports/`, `hunter-photos/` | `GET /api/media/private/<key>` を Worker が認可(§7)してストリーム。`Cache-Control: private, max-age=300`。必要なら `aws4fetch` で 5 分署名 URL も発行可 |
...
- `danger_reports.image_url → image_key`、`processed_image_urls → processed_image_keys(JSON)`、`profiles.avatar_url → avatar_key`、`report_images.image_url → image_key`、`hazard_image_cache.public_url` 削除(`storage_path` を `object_key` に改名)
...
| `ProcessImageDialog.tsx:77` クライアント直アップロード(`processed_images`) | 新設 `POST /api/admin/reports/{id}/processed-image`(admin 限定)。バケット名不一致バグはここで解消 |
...
| 署名 URL(`lib/danger-report-image-access.ts`, `safety-quest/challenges`, `hunter/storage.ts`) | `/api/media/private/<key>` 経由に統一。クライアントは `<img src="/api/media/private/...">` でよい |
...
- `hunter-photos/` は現行の 90 日保持を **R2 Object Lifecycle(prefix ルール)** で自動化
...
| 新規 | `0 18 * * *` D1 export → R2 `backups/d1/YYYY-MM-DD.sql.gz`、`0 19 * * 0` R2 孤児掃除 |
...
1. `pg_dump --schema-only` / `--data-only` を取得し R2 `backups/pre-migration/` へ保存(破棄テーブル含む全量)
...
3. Supabase JWT 署名鍵を ES256 へ移行(§7.1)。この時点でアプリ影響なし
...
1. `MAINTENANCE_MODE=read_only` を有効化(ミドルウェアで書き込み系ルートを 503 + 画面バナー)
...
| Phase | 内容 | 完了条件 |
|---|---|---|
| 0 スパイク(1 週) | OpenNext ビルド、Images binding で sharp 代替の PoC、D1 に `traffic_accidents` を入れて bbox 性能測定 | P0-1〜6 が全て緑。性能: 東京 23 区 bbox 5,000 件が p95 < 300ms |
| 1 基盤(1 週) | `wrangler.jsonc`、`lib/db/{client,schema,authz}`、Drizzle マイグレーション、vitest の better-sqlite3 ハーネス、`lib/auth/actor.ts`(JWKS 検証) | 認可マトリクステスト緑、`tsc --noEmit` 0 |
...
| `SUPABASE_JWT_ISSUER`(= `https://<project>.supabase.co/auth/v1`) | JWKS 検証 |
...
- Supabase Auth: JWKS `/auth/v1/.well-known/jwks.json`、ES256 推奨、HS256 からゼロダウンタイム移行可、JWKS キャッシュ 10 分
