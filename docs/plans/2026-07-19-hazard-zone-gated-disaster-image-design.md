...
**ゲートは deny-by-default なので、データ未整備のまま enforce にすると浸水系生成が全域で止まる。Phase 0 のデータ整備が機能リリースの前提**。
...
- サイズ対策: 全国一括は行わない。**サービス対象地域（初期: 青森県）に限定して取り込み**、`ST_SimplifyPreserveTopology`（許容誤差 ~5m）で軽量化してから投入する。判定は点包含なので5m簡略化の影響は境界近傍のみ（既知の制約として記録）
...
|---|---|---|---|
| 0 | `off` | マイグレーション適用（RPC・coverage・gate_log・statsスナップショット）+ 対象地域の A31/A40 データ変換・投入・coverage登録 + `lib/hazard-zone-gate.ts` 実装 | 対象地域の代表地点（浸水区域内/外/県外）でRPC実測が期待通り。`hazard_zones` 件数・coverage が投入記録と一致 |
...
- DBサイズ: 青森県スコープのA31/A40簡略化ポリゴンで数十MB規模を想定。全国展開時は地域分割投入とジオメトリ簡略化で管理（Phase 0 実測後に確定値を本書へ追記）

## 11. 実装順序（参考。実装は別途承認後）

1. マイグレーション（RPC・coverage・gate_log）+ RPC characterization テスト
2. `lib/hazard-zone-gate.ts`（純関数核 + 文言）+ ユニットテスト
3. A31/A40 データ変換・投入（import script 拡張 → 対象地域投入 → coverage登録）→ Phase 0 実測
